<template>
  <div class="flex flex-col items-center w-full h-full bg-gray-900">
    <!-- Header with subtle dark bluish gradient -->
    <div class="w-full bg-gradient-to-r from-slate-900 via-blue-gray-800 to-slate-800 py-10 flex flex-col items-center">
      <div
    ...
    <!-- Quick Actions / Stats -->
    <div class="max-w-7xl w-full grid grid-cols-1 sm:grid-cols-2 gap-6 mt-12 px-4 sm:px-6 lg:px-8">
      <!-- Favourites -->
      <router-link
        to="/favourites"
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-red-400" fill="currentColor" viewBox="0 0 24 24">
...
const username = userState.username; // Ref<string>
...
const fetchFavourites = async () => {
  try {
    const res = await axios.get("http://localhost:5000/favourites", axiosConfig);
